...
resource "aws_ecr_repository" "get_todo_item" {
  name = "${local.prefix}-get-todo"
}

resource "null_resource" "get_todo_item_state" {
  triggers = {
    python_file = md5(file("${path.module}/../app/get_todo_item/lambda/function.py"))
    # requirements_file = md5(file("${path.module}/../app/get_todo_item/lambda/function.py"))
    docker_file = md5(file("${path.module}/../app/get_todo_item/Dockerfile"))
  }
...
data "aws_ecr_image" "get_todo_item" {
  depends_on = [
    null_resource.get_todo_item_state
  ]

  repository_name = aws_ecr_repository.get_todo_item.name
  image_tag       = 1
}
